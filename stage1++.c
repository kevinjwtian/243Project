...
#define counter_max ((volatile int) 10)
...
int main(void) {
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
    ...
    volatile int lane = 230;// init x posn of car
    long PBreleases;
    int i;
    //*PB_EDGECAPTURE = 0;
    int key;
    ...
    points[1].y = 0;
    struct point block_one;
    bool update_one = false;
    int block1x; //holds x posn of block1
    int block2x; //holds x posn of block2

    
    //counters
    int counter_one = 10; //spawning block
    //intialize two interupt counters
    int counter_left = 0; 
    int counter_right = 0; 
    ...
    while (1) { //in game loop
        ...
        *PB_EDGECAPTURE = PBreleases;
        ...
        //update variable based to interupt    
        if(lane_change){
            if(counter_right > 0){
                lane += 6;
                car_center.x = lane;
                counter_right --;
                if((counter_right>6)||(counter_right>1&&counter_right<5))
                *RLEDS = 0x7;
                else
                *RLEDS = 0;
                ...
                car_center.x = lane;
                counter_left --;
                if((counter_left>6)||(counter_left>1&&counter_left<5))
                *RLEDS = 0x380;
                else
                *RLEDS = 0;
            }
            else{//done lane change
                *RLEDS = 0;
                lane_change = false;
            }
        }
        ...
        //block related
        if(counter_one==0&&!update_one){//spwan a block
            
            counter_one = 10; //reset counter
            update_one = true; 
            points[1].dy = 1;
            points[1].y = 0;
             } 
         ...
         //obstacles
        else if(update_one){//struct point block_one;
            //draw a obstacle(always draw in range)
            points[1].x = 170;
            draw_block(points[1]);
            if(points[1].y>242)
            update_one = false;
...
void draw_block (struct point p) {
    int box_size = 29;
    for (int x = p.x-box_size; x <= p.x+box_size; x++) {
